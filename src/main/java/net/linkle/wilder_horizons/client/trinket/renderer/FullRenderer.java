...
/** Full body/armor renderer */
public class FullRenderer implements TrinketRenderer {
	
	private static final ModelPart FULL;
    
    private final Identifier texture;
    
    /** @param texture file name in wilder_horizons/textures/models/clothing */
    ...
    public void render(ItemStack stack, SlotReference slotReference, EntityModel<? extends LivingEntity> contextModel,
        ...
        if (entity instanceof AbstractClientPlayerEntity player) {
            ...
            FULL.copyTransform(model.leftArm);
            FULL.render(matrices, consumer, light, OverlayTexture.DEFAULT_UV);
        }
    }

    static {
        var model = BipedEntityModel.getModelData(new Dilation(0.3f), 0);
        FULL = TexturedModelData.of(model, 64, 32).createModel();
    }
}
